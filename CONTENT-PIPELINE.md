...
| 3 | Synthesis / Ingest | 資料整合 | 原料 + annotation | KB Wiki 頁面（Source / Concept / Entity） |
| 4 | Atomic Content | 資料輸出（原子文章） | KB / annotation / 訪談音檔 | **Single source of truth canonical content**（手寫心得 / LLM 輔助稿 / transcribe 字幕檔） |
...
| Line 2 讀書心得 | **修修在 Project 頁面手寫的心得文** | 修修本人手寫（**最珍貴的原始輸出**，非 LLM 產） |
| Line 3 文獻科普 | LLM 輔助 outline + 修修加工的稿 | Robin 抽 KB → outline → Brook compose → 修修審稿 |

**關鍵設計原則**：Line 2 心得是修修自己的聲音，不可被 LLM 取代。系統角色限於「整合素材到 KB（Stage 3）」+「Stage 5 之後的 channel 製作」，**不介入手寫過程**。修修這週會把流程跑一次，後續再決定要不要加輔助功能（例：annotation 跳轉 / KB 引用插入）。
...
| **Line 1b Podcast 訪問新書作者** | n/a (有錄音 + 有書) | ⬜ Line 2 閱讀流程 | ⬜ Line 2 整合流程 | ✅ transcribe → 字幕檔（atomic） | 同上 | 同上 |
| **Line 2 讀書心得** | n/a (有書) | ⬜ 中/英書 + annotation（雙語閱讀） | ⬜ 書 + annotation 整合 KB | ⬜ **修修 Project 頁面手寫**（流程未跑過） | ✅ WP only / ❌ 其他 channel | ❌ |
| **Line 3 文獻科普** | ⬜ Zoro topic discovery | n/a | ⬜ 主題 retrieval → outline | ⬜ kb-synthesize skill → LLM 輔助稿 | ✅ WP only / ❌ 其他 channel | ✅ SEO 中控台 |
...
| 書類型 | Stage 2 路徑 |
|---|---|
| 中文書 | Robin Reader 直接讀 + annotation |
| 英文書 | Robin Reader 雙語閱讀 + annotation |

雙語閱讀已在 PR #71 實作，但 P2A BabelDOC（學術 PDF）+ P2B Docling（掃描書）還是缺口。Annotation 回寫到 KB 的 owner 完全沒設計（觀察 #3）。
...
| **Brook** (Composer) | n/a | n/a | n/a | ✅ Line 1 transcribe→line1_extractor / Line 3 compose（LLM 輔助稿） | ✅ FB/IG/Blog renderer + repurpose engine + **script_video pipeline（影片 channel 工具，非獨立 line）** | n/a | ✅ SEO audit + enrich (對既有文章) |
