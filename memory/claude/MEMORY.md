...
- [reference_sandcastle_token_rotation.md](reference_sandcastle_token_rotation.md) — **Sandcastle `.sandcastle/.env` GH_TOKEN 過期徵兆 + 5 分鐘 rotate**：401 Bad credentials → preprocessor abort 0 iteration；fix = `gh auth token` 從 host keyring 取現用 token 覆寫 `.env`，Claude 自動跑時不該動 user credentials
