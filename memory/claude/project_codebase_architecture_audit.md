...
**No-op verified（framing 誤判 / ROI 偏低 / docstring-only fix）2026-04-28**：
- ③ kb_writer dispatcher — audit 用「SHALLOW interface 隱藏 deep impl」當 bug 描述但 Ousterhout deep module 正是 small interface + large impl 推崇範式（誤判 #6 / 術語反向）；ADR-011 §3.5 凍結 dispatcher 設計、caller `_execute_concept_action` 也乾淨；真 friction 只 update_merge 的 LLM cost 不透明 → docstring 1 行修，不動架構（PR #220）
...
**audit skill 教訓（6 次驗證後，半數誤判）**：12 候選裡 6 個 framing 誤判（②③④⑥⑨⑩）—
...
| Ousterhout 術語反向 | 「SHALLOW interface 隱藏 deep impl」當 bug | deep module 正是 small interface + large impl，是好事；leaky abstraction 才是 docstring fix 的對象（③） |
...
**meta 規律**：audit 真正命中的是 **cohesion 錯位 / 漏 close-the-loop / OS-specific bug / re-export 缺漏 / 過時 deprecation**，都是 OOP / 收尾課題；不是 type system 或 abstraction-degree 課題；Ousterhout 術語要 sanity check（深模組是好事）。
...
5. Ousterhout 術語前 sanity check：「small interface + large impl」是深模組（好事），不是 bug；改寫成「leaky abstraction」才修架構，cost 不透明可能只要 docstring

**Why**：audit 的價值不在「找一次」，在「baseline 對照下次掃」+ ledger 累積讓 skill bias 變看得見。半數誤判率（6/12）是顯著訊號 — 下次跑 skill **必先**用上面 5 步驟過篩，再決定要不要動 code。

**How to apply**：要動架構任務前先讀這份。已 done 的不重新分析；no-op verified 的不重評；剩 ⑧ 等 Chopper。重跑整個 audit 的觸發條件：sprint boundary、加新 agent（Chopper）、完成 backlog ≥3 項。
