...
Related: `feedback_review_skill_default_for_focused_pr.md` (Claude-authored side: auto-invoke `/review` on own focused PRs without asking) · `feedback_auto_merge_after_review.md` (post-review action: squash-merge without re-asking permission) · `feedback_pr_review_merge_flow.md` (full review→merge automation flow) · [[smoke-verify-after-pr-chain]] (similar discipline boundary — what to do after pushing code).
