...
- 🚧 **PR #135 open** — `cleanup(seo): reuse Franky GSC sa (drop GSC_SERVICE_ACCOUNT_JSON_PATH)`：env key rename → `GCP_SERVICE_ACCOUNT_JSON`（reuse ADR-007 convention）+ deprecate `gsc-oauth-setup.md` → redirect `setup-wp-integration-credentials.md` + ADR-009 註明 sa 重用。CI green，mergeable CLEAN，等修修 merge。「拒絕重工 + 變胖變複雜」是最高指導原則，見 [feedback_prior_art_includes_internal_setup.md](feedback_prior_art_includes_internal_setup.md)
