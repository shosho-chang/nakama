...
# source of truth 是 gateway/handlers 的 registry + .env token — 有 handler 又有
# token pair 的 agent 自動啟動，不用改這個檔。
_BOT_TOKEN_KEY = re.compile(r"^([A-Z]+)_SLACK_BOT_TOKEN$")
...
def _discover_bots() -> list[tuple[str, str, str]]:
    """掃 env 找出所有有設 token 且有註冊 handler 的 agent bot。

    回傳 [(agent_name, bot_token, app_token), ...]，按 agent_name 字母排序以確保
    log/啟動順序穩定。三道 guard：
    - BOT_TOKEN 與 APP_TOKEN 必須成對（半設視為未上線，skip）
    - token 不能是純空白
    - `get_handler(agent)` 必須非 None（沒 handler 起 bot 也接不到訊息）
    """
    found: dict[str, tuple[str, str]] = {}
    for key, raw in os.environ.items():
        m = _BOT_TOKEN_KEY.match(key)
        if not m:
            continue
        agent = m.group(1).lower()
        bot_token = (raw or "").strip()
        if not bot_token:
            continue
        app_token = os.environ.get(f"{m.group(1)}_SLACK_APP_TOKEN", "").strip()
        if not app_token:
            logger.warning(f"[{agent}] BOT_TOKEN 有但 APP_TOKEN 缺，skip")
            continue
        if get_handler(agent) is None:
            logger.warning(f"[{agent}] token 齊了但 gateway/handlers 沒註冊 handler，skip")
            continue
        found[agent] = (bot_token, app_token)
    return [(a, b, p) for a, (b, p) in sorted(found.items())]
...
def run() -> None:
    ...
    def _run_handler(agent_name: str, handler: SocketModeHandler) -> None:
        try:
            handler.start()
        except BaseException as e:
            crashed[agent_name] = e
            logger.exception(f"[{agent_name}] Socket Mode handler crashed")
        finally:
            crash_event.set()
    ...
    # 任何 bot thread 退出就觸發 event；不容忍「gateway 活著但某隻 bot 失聯」這種
    # 半死狀態，讓 systemd 直接重啟整個 process。
    crash_event.wait()
    dead = [name for name, t in threads if not t.is_alive()]
    raise RuntimeError(f"Bot thread(s) died: {dead}. Exceptions: {crashed}")
