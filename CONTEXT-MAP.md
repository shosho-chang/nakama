...
- **Brook** — Composer（**對內加工**）：將素材 compose 成各平台格式（Blog / IG / YouTube / Newsletter）+ Podcast Subtitle V2 + 既有部落格 SEO audit / enrich — 處理「已知/已存在」的內容；Podcast 字幕子 context 見 [agents/brook/podcast_subtitles/CONTEXT.md](agents/brook/podcast_subtitles/CONTEXT.md)，repurpose 詞彙見 [agents/brook/CONTEXT.md](agents/brook/CONTEXT.md)
...
- **Shared kernel** (`shared/`、`agents/base.py`) — Agent / Run / Memory / Event / API call / Token cost；任何 agent 必經介面。**ADR-052 邊界規則（原編號 ADR-050，2026-07-25 改號）：`shared/` 只收 2+ agent 共用的基礎設施**；單一 agent 的 domain logic 住該 agent 的 package（首例：Source Promotion 19 模組 → `agents/robin/promotion/`）。跨層 contract types（`shared/schemas/`）留 kernel
...
- **「Project」** = **一組任務的集合**，任務的分組鍵（filename prefix 「{project} - {task}」+ frontmatter `projects:` 雙寫）。**跨度不進模型**——一集節目、一支影片、一條經營一年的社群戰線都是 project，差別只在裡面有哪些任務（修修 2026-09-10 裁決：「你不要管一個 project 會執行多久」）。退役的是 ADR-031 的「七道工序 workspace＋手動勾階段」那套 UI，不是單件產出這個粒度；工序真相仍住在 packaging / podcast pipeline
