...
- **「Project」** = 長期戰線（自由艦隊社群、電子報、課程…）— 把跨週任務綁在一起的容器，任務的分組鍵（filename prefix 「{project} - {task}」+ frontmatter `projects:` 雙寫）。**不是**單件產出（一支影片／一篇文）— ADR-031 的「單支影片七道工序 workspace」語意於 2026-08-31 grill 作廢，單件工序真相住在 packaging / podcast pipeline
