...
def test_opening_badge_and_one_per_transition() -> None:
    ...
    overlays = derive_brand_badge_overlays(
        components=components, duration_sec=693.1, format="long"
    )
...
def test_badge_is_clamped_by_the_end_of_the_cut() -> None:
    ...
    overlays = derive_brand_badge_overlays(
        components=components, duration_sec=108.0, format="long"
    )
...
def test_short_format_has_its_own_brand_treatment() -> None:
    ...
    assert (
        derive_brand_badge_overlays(
            components=components, duration_sec=90.0, format="short"
        )
        == ()
    )
