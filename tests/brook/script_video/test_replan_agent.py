"""Tests for ``agents.brook.script_video.replan_agent`` (ADR-038 §D3 + §D6).
...
from agents.brook.script_video.beat_editor import ReplaceQuote, SetLayout
from agents.brook.script_video.replan_agent import (
    MAX_TOOL_ITERATIONS,
    render_storyboard,
    run,
)
...
def test_cli_replan_beat_runs_end_to_end(tmp_path, monkeypatch):
    """`python -m agents.brook.script_video --episode <id> replan-beat 7 --note "..."` smoke.
