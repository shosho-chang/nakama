"""Tests for ``agents.robin.promotion.dry_run_matcher.DryRunConceptMatcher`` (N518b).
...
def test_dry_run_matcher_module_does_not_import_anthropic():
    """Subprocess: import ``agents.robin.promotion.dry_run_matcher`` and assert
    ``anthropic`` is not in ``sys.modules``."""
    code = (
        "import sys, importlib;"
        "importlib.import_module('agents.robin.promotion.dry_run_matcher');"
