"""Behavior tests for ``agents.robin.promotion.concept_promotion_engine`` (ADR-024 Slice 6 / #514).
...
FIXTURES_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "concept_promotion"
...
def test_no_book_storage_import():
    """T11: importing ``agents.robin.promotion.concept_promotion_engine`` must NOT pull
    ``shared.book_storage`` into ``sys.modules``."""
    src = textwrap.dedent(
        """
        import sys
        import agents.robin.promotion.concept_promotion_engine  # noqa: F401
...
def test_no_runtime_imports_forbidden():
    """T12: importing ``agents.robin.promotion.concept_promotion_engine`` must NOT pull
