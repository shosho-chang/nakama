"""Behavior tests for ``agents.robin.promotion.promotion_commit`` (ADR-024 Slice 7 / #515).
...
to ``sys.executable`` to assert ``agents.robin.promotion.promotion_commit`` /
``agents.robin.promotion.promotion_acceptance_gate`` import surfaces stay clean.
...
from agents.robin.promotion.promotion_commit import (
    FilesystemKbWriteAdapter,
    PromotionCommitService,
)
from agents.robin.promotion.promotion_renderer import render_concept_page, render_source_page
...
FIXTURE_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "promotion_commit"
...
def test_no_book_storage_import():
    """T12: importing promotion_commit / promotion_acceptance_gate (under
    agents.robin.promotion) must NOT pull shared.book_storage into sys.modules."""
    src = textwrap.dedent(
        """
        import sys
        import agents.robin.promotion.promotion_commit  # noqa: F401
        import agents.robin.promotion.promotion_acceptance_gate  # noqa: F401
        import agents.robin.promotion.promotion_renderer  # noqa: F401
