"""Behavior tests for ``agents.robin.promotion.promotion_preflight`` (ADR-024 Slice 3 / #511).
...
def test_preflight_inspector_error_falls_back_to_defer(caplog):
    ...
    caplog.set_level("WARNING", logger="nakama.agents.robin.promotion.promotion_preflight")
...
def test_preflight_no_llm_client_import():
    """T11: importing agents.robin.promotion.promotion_preflight must NOT pull anthropic /
    openai / google.generativeai into sys.modules — confirms preflight is
    deterministic per CONTEXT.md ('without heavy LLM spend')."""
    src = textwrap.dedent(
        """
        import sys
        import agents.robin.promotion.promotion_preflight  # noqa: F401
...
def test_preflight_no_fastapi_or_thousand_sunny_imports():
    """T12: importing agents.robin.promotion.promotion_preflight must NOT pull fastapi,
...
def test_preflight_no_book_storage_import():
    """T14: importing agents.robin.promotion.promotion_preflight must NOT pull
...
def test_count_vtt_cues_handles_inline_tags_and_NOTE_blocks():
    ...
    from agents.robin.promotion.promotion_preflight import _count_vtt_cues_and_text
