"""Behavior tests for ``agents.robin.promotion.source_map_builder`` (ADR-024 Slice 5 / #513).
...
FIXTURES_DIR = Path(__file__).resolve().parents[3] / "fixtures" / "source_map"
...
def test_build_does_not_call_real_llm():
    """T5: subprocess assertion that ``agents.robin.promotion.source_map_builder`` import does
    NOT pull anthropic / openai / claude_client / google.generativeai modules
    into ``sys.modules``."""
    src = textwrap.dedent(
        """
        import sys
        import agents.robin.promotion.source_map_builder  # noqa: F401
...
def test_build_excerpt_total_below_30pct_of_chapter():
    ...
    from agents.robin.promotion.source_map_builder import _strip_xml
    from shared.epub_metadata import extract_spine_items
...
def test_no_book_storage_import():
    """T11: importing ``agents.robin.promotion.source_map_builder`` must NOT pull
...
def test_no_runtime_imports_forbidden():
    """T12: importing ``agents.robin.promotion.source_map_builder`` must NOT pull fastapi,
