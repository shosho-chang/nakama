...
def test_surprised_evidence_prefers_warm_thoughtful_over_loud_reaction(fake_manifest):
    manifest_path, vault_root = fake_manifest
    ...
    request = build_cast_request_from_idea(
        {
            ...
            "visual": "warm explainer research evidence card",
...
def test_shosho_benefit_card_prefers_presenter_pose_over_warning(tmp_path: Path):
    vault_root = tmp_path / "vault"
    manifest = {
        "schema_version": "shosho_cutout_pose_manifest.v1",
        "entries": [
            _entry(
                vault_root,
                "C_WARNING",
                "Attachments/cutouts/shosho/serious/1.png",
                expression_family="serious_warning",
                intensity="mild",
                credibility="high",
                use_context=["warning", "evidence_review"],
                confidence=0.95,
                hands="arms_crossed",
            ),
            _entry(
                ...
                intensity="subtle",
                credibility="high",
                use_context=["personal_story", "evidence_review", "ali_warm_explainer"],
                confidence=0.92,
                hands="chin",
            ),
            _entry(
    ...
    assert "warning" in request.avoid_contexts
    assert [candidate.cutout_id for candidate in candidates] == ["C_PRESENT", "C_THINK"]
