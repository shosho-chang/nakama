"""Slice 4B — POST ``/robin/api/books/{id}/ingest-request`` API contract.
...
- The route MUST also gate behind the existing ``check_auth`` cookie like the
  rest of ``/robin/api/books/*``.

The ``GET /robin/api/books/{book_id}`` route extension (added in this slice for the
...
def test_post_ingest_request_idempotent_double_post(app_client):
    ...
    _upload(app_client, "alpha", with_original=True)
    r1 = app_client.post("/robin/api/books/alpha/ingest-request")
    r2 = app_client.post("/robin/api/books/alpha/ingest-request")
...
def test_post_ingest_request_book_missing_returns_404(app_client):
    r = app_client.post("/robin/api/books/nonexistent/ingest-request")
    assert r.status_code == 404


# ---------------------------------------------------------------------------
# GET /robin/api/books/{id} — extended with ingest_status field
# ---------------------------------------------------------------------------


def test_get_book_includes_ingest_status_never(app_client):
    _upload(app_client, "alpha", with_original=True)
    body = app_client.get("/robin/api/books/alpha").json()
    assert body["ingest_status"] == "never"


def test_get_book_includes_ingest_status_queued(app_client):
    _upload(app_client, "alpha", with_original=True)
    app_client.post("/robin/api/books/alpha/ingest-request")
    body = app_client.get("/robin/api/books/alpha").json()
...
# ---------------------------------------------------------------------------
# DELETE /robin/api/books/{id}/ingest-request — cancel queued ingest
# ---------------------------------------------------------------------------
...
def test_delete_ingest_request_cancels_queued(app_client):
    ...
    r = app_client.delete("/robin/api/books/alpha/ingest-request")
    assert r.status_code == 200
    assert app_client.get("/robin/api/books/alpha").json()["ingest_status"] == "never"
...
def test_delete_ingest_request_refuses_when_ingesting(app_client):
    ...
    r = app_client.delete("/robin/api/books/alpha/ingest-request")
    assert r.status_code == 409
    assert app_client.get("/robin/api/books/alpha").json()["ingest_status"] == "ingesting"


# ---------------------------------------------------------------------------
# DELETE /robin/api/books/{id} — full book removal
# ---------------------------------------------------------------------------
...
def test_delete_book_removes_row_files_and_queue(app_client, books_dir):
    ...
    r = app_client.delete("/robin/api/books/alpha")
    assert r.status_code == 200
    assert app_client.get("/robin/api/books/alpha").status_code == 404
    assert not (books_dir / "alpha").exists()


def test_delete_book_404_when_missing(app_client):
    r = app_client.delete("/robin/api/books/nonexistent")
    assert r.status_code == 404


# ---------------------------------------------------------------------------
# GET /robin/api/books/{id}/cover — bytes from EPUB extracted at upload time
# ---------------------------------------------------------------------------


def test_get_cover_404_when_no_cover_in_epub(app_client):
    _upload(app_client, "alpha", with_original=True)
    r = app_client.get("/robin/api/books/alpha/cover")
    assert r.status_code == 404
...
def test_get_cover_returns_image_bytes_from_epub_with_cover(app_client):
    ...
    r = app_client.post("/robin/books/upload", data=data, files=files)
