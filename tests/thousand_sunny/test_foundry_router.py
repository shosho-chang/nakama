...
from thousand_sunny.routers import foundry
...
def isolated_root(tmp_path, monkeypatch):
    ...
    foundry._set_data_root(tmp_path / "data")
    monkeypatch.setattr(edit_log, "_LOG_DIR", tmp_path / "edit_log")
    monkeypatch.setattr(foundry, "_EXAMPLES_DIR", tmp_path / "examples")
    return tmp_path
...
def render_calls(monkeypatch):
    ...
    monkeypatch.setattr(foundry, "_render_beat", _fake)
    return calls
...
def test_status_endpoint_returns_chip_payload(client, isolated_root):
    ...
    res = client.get("/foundry/ep-2/status")
    assert res.status_code == 200
    data = res.json()
...
def test_approve_sets_text_approved_and_enqueues(client, isolated_root, render_calls):
    ...
    res = client.post("/foundry/ep-3/beat/5/approve", follow_redirects=False)
...
def test_batch_approve_all_text_marks_all_pending(client, isolated_root, render_calls):
    ...
    res = client.post("/foundry/ep-6/batch/approve-all-text", follow_redirects=False)
    assert res.status_code == 303
    saved = yaml.safe_load(sb.read_text(encoding="utf-8"))
...
def test_batch_render_approved_reenqueues_failed(client, isolated_root, render_calls):
    ...
    res = client.post("/foundry/ep-7/batch/render-approved", follow_redirects=False)
    assert res.status_code == 303
    queued = {bid for _, bid in render_calls}
...
def test_batch_finalize_passing_sets_visual_approved(client, isolated_root):
    ...
    res = client.post("/foundry/ep-8/batch/finalize-passing", follow_redirects=False)
    assert res.status_code == 303
    saved = yaml.safe_load(sb.read_text(encoding="utf-8"))
...
def test_visual_replan_writes_edit_log_with_visual_tag(client, isolated_root):
    ...
    res = client.post(
        "/foundry/ep-9/beat/4/visual/replan",
        data={"note": "color off"},
        follow_redirects=False,
    )
    assert res.status_code == 303
...
def test_promote_to_example_writes_yaml_and_index(client, isolated_root):
    ...
    client.post("/foundry/ep-10/beat/8/replan", data={"note": "promote-me"}, follow_redirects=False)
    res = client.post(
        "/foundry/ep-10/edit-log/0/promote",
        data={"tag": "stat-callout"},
        follow_redirects=False,
    )
    assert res.status_code == 303
    examples_dir = foundry._EXAMPLES_DIR
...
def test_storyboard_404_when_missing(client, isolated_root):
    res = client.get("/foundry/does-not-exist")
