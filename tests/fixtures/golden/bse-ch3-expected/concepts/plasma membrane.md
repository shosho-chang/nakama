---
schema_version: 3
title: plasma membrane
type: concept
domain: biochemistry
aliases: []
mentioned_in:
- '[[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch3]]'
- '[[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch6]]'
source_refs: []
discussion_topics: []
confidence: null
tags: []
created: 2026-05-08
updated: 2026-05-08
en_source_terms:
- plasma membrane
- Plasma membrane
maturity_level: L2
high_value_signals:
- section_heading
- bolded_define
- freq_multi_section
---

## Definition

This concept area covers the foundational chemistry and biochemistry relevant to human physiology, sport and exercise, encompassing atomic and molecular structure, chemical reactions, cellular structure, and the structure and metabolism of lipids.

## Core Principles

**Atomic and molecular foundations:**
* identify the main chemical elements in the human body;
* describe and draw basic atomic structure;
* identify the differences between atoms, ions, molecules and compounds;
* distinguish the difference between ionic and covalent bonds;

**Chemical reactions and energy:**
* define a chemical reaction and identify the difference between an endergonic and exergonic reaction;
* define energy and the units of energy;
* describe the role of ATP within the cell;
* describe the common forms of chemical reactions including synthesis, decomposition, reversible, exchange, phosphorylation/dephosphorylation and oxidation/reduction reactions;

**Water, solutions and acid–base chemistry:**
* outline the functions of water and the chemical property underpinning water's ability to act as a solvent;
* define the terms mole, molar and molarity;
* define the terms acid, base and salt;
* draw and define the pH scale and describe what is meant by the terms acidic and alkaline;

**Cell structure:**
* describe basic cell structure with particular emphasis to the plasma membrane, nucleus, cytoplasm and mitochondria.

**Lipid structure and metabolism:**
* draw the general structure of a triacylglycerol molecule;
* draw the structure of a saturated and an unsaturated fatty acid molecule;
* outline differences in the types of lipoproteins;
* describe the structure of a cholesterol molecule and its similarity to steroid hormones;
* describe the processes of lipolysis, β-oxidation and lipogenesis;
* list the key enzymes regulating the above processes;
* explain how fatty acids are transported in the blood and across both the plasma membrane and the mitochondrial membrane;
* calculate the number of ATP molecules produced from the oxidation of a fatty acid molecule;
* describe the formation of a ketone body;
* describe how excess carbohydrates can be converted to fatty acids.

## Sub-concepts

* Atomic structure, ions, molecules, compounds
* Ionic vs covalent bonding
* Endergonic vs exergonic reactions; ATP energetics
* Reaction types: synthesis, decomposition, reversible, exchange, phosphorylation/dephosphorylation, oxidation/reduction
* Water as solvent; mole, molar, molarity
* Acids, bases, salts; pH scale
* Cell organelles: plasma membrane, nucleus, cytoplasm, mitochondria
* Triacylglycerols; saturated and unsaturated fatty acids
* Lipoproteins; cholesterol and steroid hormone analogues
* Lipolysis, β-oxidation, lipogenesis and their regulatory enzymes
* Fatty acid transport (blood, plasma membrane, mitochondrial membrane)
* ATP yield from fatty acid oxidation
* Ketone body formation
* Conversion of excess carbohydrates to fatty acids

## Field-level Controversies

## 文獻分歧 / Discussion

## Practical Applications

* Functions of lipids within the human body
* Role of lipids in sport and exercise

## Related Concepts

## Sources

- [[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch6]]
- [[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch3]]
