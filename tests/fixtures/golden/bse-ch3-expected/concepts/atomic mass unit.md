---
schema_version: 3
title: Atomic mass unit
type: concept
domain: biochemistry
aliases: []
mentioned_in:
- '[[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch3]]'
source_refs: []
discussion_topics: []
confidence: null
tags: []
created: 2026-05-08
updated: 2026-05-08
en_source_terms:
- Atomic mass unit
- atomic mass unit
maturity_level: L2
high_value_signals:
- bolded_define
---

The *atomic mass* (often referred to as *atomic weight*) of an element is the average mass of all its naturally occurring isotopes. The standard unit for atomic mass (*atomic mass unit*, *abbreviated as ‘amu’*) is the *dalton*, which is symbolized as Da. The mass of a neutron is 1.008 Da, the mass of a proton is 1.007 Da and the mass of an electron is 0.0005 Da. In its purest sense, the Da can be defined as one-twelfth of the mass of a carbon 12C atom and therefore equates to the extremely small mass of 1.66 × 10−24 g! When rounded to the nearest whole number, the atomic mass of an element typically coincides with the mass number of the predominant isotope of that element (see Figure 3.2).

![A set of eight diagrams depicts a basic diagrammatic representation of the atomic structure of several stable atoms. These atoms consist of even smaller subatomic particles: protons, neutrons, and electrons. a. A first electron shell represents the innermost shell where electrons reside. b. A second electron shell represents the shell surrounding the first electron shell. c. It represents Nitrogen, Atomic number = 7, mass number = 14 or 15, atomic mass = 14.01. d. It represents oxygen, atomic number = 8, mass number = 16, 17, or 18, and atomic mass = 16.00. e. A third electron represents the shell surrounding the second electron shell. It represents sodium, atomic number = 11, mass number = 23, atomic mass = 22.99. f. It represents chlorine, atomic number = 17, mass number = 35 or 37, and atomic mass = 35.45. g. A fourth electron shell surrounds the third electron shell. Only Sodium and Chlorine have a fourth shell in the image. h. A fifth electron shell is the outermost shell for Potassium.](Attachments/Books/biochemistry-for-sport-and-exercise-maclaren/c03f002.png)

## Definition

The atomic mass (also called atomic weight) of an element is the average mass of all its naturally occurring isotopes, expressed in atomic mass units (amu), with the dalton (Da) serving as the standard unit. One dalton is defined as one-twelfth of the mass of a carbon-12 (¹²C) atom, equivalent to approximately 1.66 × 10⁻²⁴ g.

## Core Principles

- The mass of a neutron is 1.008 Da, the mass of a proton is 1.007 Da, and the mass of an electron is 0.0005 Da, indicating that nucleons account for nearly all of an atom's mass while electrons contribute negligibly.
- Atomic mass is calculated as the weighted average of the masses of all naturally occurring isotopes of an element.
- When rounded to the nearest whole number, the atomic mass of an element typically coincides with the mass number of its predominant isotope.

## Sub-concepts

- **Atomic mass unit (amu) / Dalton (Da):** The standard unit for expressing atomic-scale masses; 1 Da = 1/12 of the mass of a ¹²C atom ≈ 1.66 × 10⁻²⁴ g.
- **Isotopes and mass number:** Different isotopes of the same element have different mass numbers (e.g., nitrogen: 14 or 15; oxygen: 16, 17, or 18; chlorine: 35 or 37), but the element's atomic mass reflects the natural abundance-weighted average (e.g., N = 14.01, O = 16.00, Na = 22.99, Cl = 35.45).
- **Subatomic particle masses:** Protons (1.007 Da) and neutrons (1.008 Da) reside in the nucleus; electrons (0.0005 Da) occupy electron shells around the nucleus.

## Field-level Controversies

## 文獻分歧 / Discussion

## Practical Applications

- Atomic masses listed on the periodic table (e.g., N = 14.01, O = 16.00, Na = 22.99, Cl = 35.45) are used directly in stoichiometric calculations, molecular weight determinations, and biochemical concentration computations.
- Recognizing that rounded atomic mass approximates the predominant isotope's mass number allows quick identification of the most abundant isotope of an element.

## Related Concepts

- Atomic number
- Mass number
- Isotopes
- Electron shells
- Subatomic particles (proton, neutron, electron)

## Sources

- [[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch3]]
