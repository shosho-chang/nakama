---
schema_version: 3
title: acid
type: concept
domain: biochemistry
aliases: []
mentioned_in:
- '[[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch3]]'
source_refs: []
discussion_topics: []
confidence: null
tags: []
created: 2026-05-08
updated: 2026-05-08
en_source_terms:
- acid
- Acid
maturity_level: L2
high_value_signals:
- section_heading
- bolded_define
- freq_multi_section
---

* identify the main chemical elements in the human body;
* describe and draw basic atomic structure;
* identify the differences between atoms, ions, molecules and compounds;
* distinguish the difference between ionic and covalent bonds;
* define a chemical reaction and identify the difference between an endergonic and exergonic reaction;
* define energy and the units of energy;
* describe the role of ATP within the cell;
* describe the common forms of chemical reactions including synthesis, decomposition, reversible, exchange, phosphorylation/dephosphorylation and oxidation/reduction reactions;
* outline the functions of water and the chemical property underpinning water’s ability to act as a solvent;
* define the terms mole, molar and molarity;
* define the terms acid, base and salt;
* draw and define the pH scale and describe what is meant by the terms acidic and alkaline;
* describe basic cell structure with particular emphasis to the plasma membrane, nucleus, cytoplasm and mitochondria.

---

## Definition

A foundational chapter outlining the basic chemistry and cell biology relevant to sport and exercise biochemistry. It covers the chemical elements of the human body, atomic structure, the distinctions between atoms, ions, molecules and compounds, the nature of chemical bonds and reactions, energy and its units, the role of ATP, the properties of water, acid–base chemistry, and basic cell structure.

## Core Principles

- The human body is composed of a defined set of main chemical elements.
- Atoms have a basic structure (nucleus with protons and neutrons, surrounded by electrons) that can be described and drawn.
- Atoms, ions, molecules and compounds are distinct entities with specific definitions.
- Chemical bonds include ionic bonds (transfer of electrons) and covalent bonds (sharing of electrons).
- A chemical reaction involves the rearrangement of atoms; reactions are classified as endergonic (energy-absorbing) or exergonic (energy-releasing).
- Energy is defined and quantified using specific units of energy.
- ATP serves as the principal energy currency within the cell.
- Common reaction types include synthesis, decomposition, reversible, exchange, phosphorylation/dephosphorylation, and oxidation/reduction reactions.
- Water functions as a biological solvent due to its underlying chemical properties (e.g., polarity).
- Acids, bases and salts are defined by their behaviour in solution; the pH scale quantifies acidity and alkalinity.
- Basic cell structure includes the plasma membrane, nucleus, cytoplasm and mitochondria, each with distinct functions.

## Sub-concepts

- Main chemical elements of the human body
- Atomic structure
- Atoms, ions, molecules, compounds
- Ionic vs covalent bonds
- Chemical reactions: endergonic vs exergonic
- Energy and units of energy
- ATP and its cellular role
- Reaction types: synthesis, decomposition, reversible, exchange, phosphorylation/dephosphorylation, oxidation/reduction
- Water as a solvent
- Mole, molar, molarity
- Acids, bases, salts
- pH scale; acidic and alkaline
- Cell structure: plasma membrane, nucleus, cytoplasm, mitochondria

## Field-level Controversies

## 文獻分歧 / Discussion

## Practical Applications

- Provides the chemical and cellular foundation required to understand metabolism, bioenergetics, and exercise biochemistry.
- Underpins quantitative work in the laboratory through the definitions of mole, molar and molarity.
- Supports interpretation of acid–base balance (pH) in physiological and exercise contexts.
- Frames the role of ATP and oxidation/reduction and phosphorylation/dephosphorylation reactions in energy transfer during exercise.

## Related Concepts

## Sources

- [[Sources/Books/biochemistry-for-sport-and-exercise-maclaren/ch3]]
