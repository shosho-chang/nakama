...
- #6 金額上限 → interactive 每日 US$5（自動）、batch 每次核准預設 US$20。
- #7 別名 → 不在 code 放對照表，改用 SDK 每週自動升級、CI 綠燈自動合併（ADR-070 D10）。
- #15 常備授權 → 選 (b)：interactive 自動轉 OpenRouter，batch 等重置或手動核准（ADR-070 D5）。
- #21 縮圖腳本 → 刪除（素材和中間產物都已不存在）。
- F18 Anthropic BYOK key → 由修修從 OpenRouter 後台移除。
