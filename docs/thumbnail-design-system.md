...
| 框型 | `frame_style: hybrid` | 斜切框＋左緣碎片 — 見〈品牌識別對接〉 |
| prop 卡幾何 | `prop_left_pct` **15**、`prop_width_pct` **52** | 左緣讓出 192px 給碎片；右緣仍塞進肩膀後 |
| 人物 glow | 36px/0.5 ＋ 90px/0.3（`person_glow_color` **`#F37425`**） | 大半徑低強度，均勻無硬邊 |
| 內側緣漸隱 | `inner_edge_fade_pct` **9** | 蓋掉裁框硬邊；mask 套在 glow 之後 |
| 頻道 logo | `logo_position: below-card`、`logo_height_px` **96** | 左下暗底（卡自動上移 46%）— **白線 logo 不可壓在 prop 卡上**（淺色卡會糊掉）。尺寸上限由淨空規範決定，見〈品牌識別對接〉 |
| 背景 | 修修正版背景圖 | `bg_image_data_url` |

**z 序**：背景 0 → 碎片 3（卡背後）→ prop 卡 4 → 人物 5 → logo 6。
...
## 品牌識別對接（**2026-07-29 修修裁決 — C 案**）

來源：`F:\Project Files\Assets\張修修品牌\張修修品牌識別_0827.pdf`。
起因是修修 2026-07-29：「Highlight 長片的封面框框跟 Chris Williamson 實在太像了」。
原本的 8px 圓角矩形橘框是通用語彙，把品牌獨有的部分全拿掉了。

### 品牌書怎麼說（頁碼 = PDF 實際頁）

| 頁 | 內容 |
|----|------|
| p7 | Logo Arrange — **最小淨空範圍**，以 X 為單位 |
| p10 | 品牌輔助色 PANTONE 165 PC |
| p15 | 品牌圖樣 A 版：正方形 →**擴張**→**自我解讀**（斜切四邊形）→**群體連結**（斜切矩形咬合鏈） |
| p16 | 圖樣 A/B 版滿版 — 碎片密度與負空間比例 |
| p22 | **Youtube 影片引用字卡** — 橘粗框 ＋ 框背後左右爆出的鋸齒碎片。**品牌書自己對「怎麼框住一張圖」的答案** |
| p24 | ICON 字卡 — 空心橘框 ＋ 兩側碎片（同一套變體） |

### accent 色差（三案共同修正）

p10 內文寫 `#e98965`，但同頁的 `CMYK 0.68.98.0` 與 `RGB 233.105.37` 互斥。
量測品牌書實際稿件像素（p10 / p15 / p16 / p22 / p24 五頁）**全部是 `#F37425`**，
與 CMYK 換算吻合 → `#e98965` 是書內筆誤（`#E96925` 打錯）。

該筆誤已從 `docs/design-system.md:35,116` 擴散到 `thumbnail_reaction` 與
`thumbnail_topic`。**縮圖家族一律用 `#F37425`**（框、碎片、人物 glow 同色）。
`docs/design-system.md`（Bridge/Web UI token）尚未跟進 — 改動會重新上色整個 UI，
待修修另行裁決。

### 斜切量的來源（不是目測）

量測 p15「自我解讀」四角（p15 頁面座標）：`(1262,376) → (1505,391) → (1502,477)`
→ 頂緣朝右下傾 17px/243px、左緣較高。等比例搬到 576px 高的卡片會是 96px 落差，
看起來像壞掉 → 收斂為 **頂 3.5% / 底 1.2%**：

```css
clip-path: polygon(0% 0%, 100% 3.5%, 100% 100%, 0% 98.8%);
```

框與圖層各自套同一個 `polygon(%)`，內層盒小 24px、斜率差 <1px →
實測邊寬 **10–11px 等寬**（x=300/450/600/750 四處取樣）。

### 碎片幾何

6 片斜切矩形（旋轉矩形多邊形），**硬寫在 composition 內、不用亂數** → 可重現。
長短交錯留負空間縫隙，模仿 p22 碎片彼此不齊的咬合。
`prop_position=left` 時**右側碎片關閉** — 卡右緣本來就躲在人物後面，
右碎片只會從肩上冒出零星角料。

### logo 尺寸上限＝淨空規範

p7 face mark panel 量測：淨空 X = 92px、logo 盒 392px → **X ≈ 0.235 × logo 高**。
以此掃尺寸（punch-L5 幾何）：

| logo 高 | 需要 X | 上緣（碎片） | 下緣（畫布） | 左緣 |
|---|---|---|---|---|
| 82（舊值） | 19px | 44 ✓ | 16 ✗ | 46 ✓ |
| **96（定案）** | **23px** | **30 ✓** | **24 ✓** | **46 ✓** |
| 108 | 25px | 14 ✗ | 24 ✓ | 46 ✓ |
...
配套：`below-card` 的 logo 下緣留白 16→**24px**（16 連舊尺寸都不夠），
最下方碎片上移 10px 讓出上緣淨空。**要再放大 logo 就必須動碎片幾何**。

### 小尺寸驗證（320×180 = YouTube 格線真實尺寸）

碎片在格線尺寸仍是一團可辨識的橘色鋸齒（有效）；斜切只剩頂緣一條斜線，
存在感弱 — 差異化的力道主要來自「**沒有圓角**」而不是「斜」。
...
**內側緣漸隱的裁切框前提（2026-08-14 鄭國威集事故）**：`inner_edge_fade_pct 9`
吃的是「元素內側 9% 寬」，不挑內容。裁切框的內側界除了要落在自然物件邊緣，
**還要留至少一個 fade 寬度的非臉部素材**（麥克風、肩膀）給它吃；把界線切在
臉緣 → 漸層直接吃臉，連沿剪影畫的橘 glow 一起衰減（實測剪影外圈 R−B 42.1
vs 對側 60.9，修法後回到 63.6 / 61.1）。驗法：對成品量剪影外圈橘度，兩人不該
差超過 ~10。
...
5. **人名／credit 帶零硬遮擋（修修 2026-08-05 硬規則）**：平衡容忍（≤600px²）
   只適用標題大字——「重要資訊像是人名，千萬不能被擋住」。credit 帶被物件
   實蓋（diff>150，例如麥克風海綿 ≈ 550）＝直接 FAIL，不論面積多小、平衡
   多漂亮；glow 半透明染色（diff 90–150）字形不受影響，另報資訊。實際事故：
   安吉集 N1 總遮蔽 409px² 過了平衡 gate，卻正好是「吉」右下角被 guest 麥
   海綿咬掉。雙 gate 腳本：
   `.claude/skills/thumbnail-brainstorm/scripts/occlusion_check.py`
