...
> **2026-07-03 補錄（recovery note）** — 本文件 2026-05-28 定稿於 `docs/adr-038-foundry-phase2` branch，但該 branch 從未 merge 進 main；期間 PR-A/B/C/G（#784/#782/#783/#800）已依本文實作並 merge，code 註解與 commit message 均引用「ADR-038」而 `docs/decisions/` 查無此文。2026-07-03 自該 branch 原文 recover 至 main，本文正文一字未改；實作狀態與正文偏差見文末 [附錄：2026-07-03 補錄](#附錄2026-07-03-補錄實作狀態與正文偏差)。
...
## 附錄：2026-07-03 補錄（實作狀態與正文偏差）

本節為 recovery 時追加，正文（2026-05-28 定稿）未動。事實依據為 merged PR diff 與 main 當下程式碼。

### 缺檔根因

`docs/adr-038-foundry-phase2` branch（HEAD `6c5388b`）從未 merge。四個實作 PR 全部引用本文（如 `agents/foundry/fcpxml_emitter.py:108`「ADR-038 §D2」、PR #784 body 直接連結本路徑），造成兩個月的 dangling reference。同 branch 上的 panel 產物四份（borrowings / codex audit / gemini audit / integration matrix）一併 recover 至 `docs/research/`；一次性 dispatch 腳本（`scripts/dispatch_{codex,gemini}_adr038.py`）留在該 branch，未納入。

### 實作狀態（截至 2026-07-03）

| PR slice | 決策 | GitHub PR | Merged | 現況 |
|---|---|---|---|---|
| PR-A | D2 export hash + `EXPORT_VERSION` | [#784](https://github.com/shosho-chang/nakama/pull/784)（issue #778） | 2026-05-28 | ✅ `shared/foundry_versions.py`、`agents/foundry/export_hash.py`、3 處檔名站點同步改 hash-named |
| PR-B | D7 LCS storyboard diff + edit_log 增補 | [#782](https://github.com/shosho-chang/nakama/pull/782)（issue #779） | 2026-05-28 | ✅ `agents/foundry/storyboard_diff.py`、`edit_log` 增 `diff` 欄位（optional、向後相容）、CLI `diff` |
| PR-C | D5 silencedetect parser + hint-beats | [#783](https://github.com/shosho-chang/nakama/pull/783)（issue #780） | 2026-05-28 | ✅ `agents/foundry/silence_detection.py`、CLI `hint-beats`、`plan --use-hints`（default opt-out，無 hints 時 planner 行為不變） |
| PR-G | D3 + D6 beat_editor + replan_agent | [#800](https://github.com/shosho-chang/nakama/pull/800)（issue #781） | 2026-06-02 | ✅ `agents/foundry/beat_editor.py`（11 ops）、`agents/foundry/replan_agent.py`、CLI `replan-beat`、Bridge endpoint 真接 LLM |
| PR-D / PR-E | D1 Resolve Python driver + render queue | — | — | ❌ 未啟動。`agents/foundry/drivers/` 不存在；Phase 1 acceptance gate（修修真實 10-15min episode end-to-end）仍未關 |
| PR-F | D4 Resolve ASR（spike 先行） | — | — | ❌ 未啟動；Mandarin spike 未跑，§OQ6 仍空 |

### 實作與正文的偏差（以 code 為準）

1. **Bridge endpoint 路徑** — 正文 Consequences §5 寫 `POST /foundry/<ep-id>/replan-beat/<beat_id>`；實際為 `POST /foundry/{episode_id}/beat/{beat_id}/replan` 與 `POST /foundry/{episode_id}/beat/{beat_id}/visual/replan`（`thousand_sunny/routers/foundry.py:308,409`）。
2. **Token budget cap 具體化** — §OQ4 只說「token budget cap」；PR #800 定為 tool-loop 5 iterations + 20k cumulative tokens。
3. **D6 嚴格拒絕 bare `[N]`** — 正文說 tool calls 接受 `[beat:N]` 或 canonical id；實作額外把 bare `[N]` 顯式 reject（`agents/foundry/beat_editor.py:77`，CJK 逗號歧義防線做滿）。
4. **`add-captions` CLI 尚不存在** — Consequences §4 的新 CLI 清單含 `add-captions`，該項屬 PR-F 範圍，隨 D4 未啟動而未 ship。
5. **LLM 失敗降級**（正文未規定、實作補上）— Bridge replan endpoint 在 LLM 失敗時仍記錄 user note 並 reset `render_status`，operator 可重發（PR #800）。

### 路徑有效性檢查（2026-07-03）

Foundry 程式碼位置 = `agents/foundry/`（本文所有路徑與 main 一致）。同日有一項「video production line 併入 Brook（`agents/brook/script_video/`）」的 ownership 提案在裁決中，草案暫定編號 ADR-050 — 注意該編號已與兩個 in-flight ADR 撞號（`ADR-050-robin-promotion-bounded-package`，PR #971；`ADR-050-zoro-fitness-coach`，`feat/zoro-coach` branch），最終編號待定。若該提案 accepted，路徑遷移與本文引用更新由該 ADR 的 PR 負責；本文不預改。
