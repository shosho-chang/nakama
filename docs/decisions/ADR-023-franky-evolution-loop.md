# ADR-023: Franky Evolution Loop V1 — 從 monitor 擴張到 evolution watch
...
ADR-001 在本 ADR commit 同步更新該 row，並在 row 下方註記 `Amended by ADR-023 (2026-05-07)`。
...
ADR-023 Phase 1 即可 ship，**不阻塞**等 ADR-020。
...
- **ADR-020 dependency 鎖**：本 ADR 不引用 ADR-020 為 production dependency；Phase 2 切換另起 ADR（暫稱 ADR-023b）
