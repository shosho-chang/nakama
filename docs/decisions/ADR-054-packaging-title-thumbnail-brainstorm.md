...
| D17 | ~~**Reject 是 revision command，不是死狀態**（2026-08-21）~~ **已撤銷（2026-09-15）**：Reject 按鈕、reject note 與 `revision_job` 整條退場。修修：「reject note 這個框框以及 reject 按鈕完全都不用了，我不知道這裡的 reject 按下去會有什麼行為」——他實際的工作方式是在 gate 上自己改標題、自己組封面。gate 只剩 Approve；`reject_note` / `revision_job` 欄位保留純為讀舊檔。 | — |
