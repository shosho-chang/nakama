...
- **Per-task optimistic lock (deferred item H) — scoped, not uniform.** The new destructive routes (reschedule/unlink/unschedule) DO carry `expected_token` (the page already has it). The broader retrofit of the older dashboard plan routes (add/remove/sync/schedule) is **deliberately deferred**: 修修 confirmed the vault is a pure substrate mutated only via Web UI / Nami (never hand-edited frontmatter), so concurrent-edit risk on those routes is minimal, and the dashboard doesn't carry per-task tokens (a uniform rollout would push a SHA-1 read onto the all-tasks hot path). Revisit if a second writer (e.g. a Nami↔Bridge race) becomes real.
- **Known v1 limitation — stale idempotency key after reschedule.** `update_event` keeps the event's `extendedProperties.lifeos_task = {slug}@{original-date}` key, so after a date change the key no longer matches the new date. Benign in v1: a linked task never re-enters the create path (its backlog picker is replaced by the locked note), and reschedule/rollback locate by event **id**, not key. A multi-block (D6-deferred) future would need the key refreshed.
