...
**實測校準（2026-04-26 半量 drill）**：state.db `restore_from_r2.py` step（gz fetch + gunzip + verify_db）實測 wall-clock <30 秒（44 KB snapshot），剩 RTO 預算（~29 min）涵蓋停 service / 完整 smoke check / 修修人為反應時間。Drill outcome：[`Shosho LifeOS/Incidents/2026/04/drill-2026-04-26-state-restore.md`]。
...
| approval_queue 表完整 | `python3 -c "import sqlite3; [print(s,n) for s,n in sqlite3.connect('data/state.db').execute('SELECT status, COUNT(*) FROM approval_queue GROUP BY status')]"` | 看到 status 統計（VPS 預設沒裝 sqlite3 CLI，用 stdlib） |
...
**Default 模式：VPS sandbox**（不影響 prod `/home/nakama/data/state.db`）。VPS 已有完整 R2 credentials + python deps。

```bash
# 1. 在 VPS sandbox 開乾淨工作目錄
ssh nakama-vps
DRILL_DIR=/tmp/dr-drill-$(date +%Y%m%d)
mkdir -p $DRILL_DIR
cd /home/nakama

# 2. 從 R2 拉最新 state snapshot（注意：必須 --apply 才會真的寫到 --target；
#    沒 --apply 是 dry-run，路徑用 TemporaryDirectory，with-block 結束就清掉）
python3 scripts/restore_from_r2.py list --db state --limit 5
python3 scripts/restore_from_r2.py restore --db state --target $DRILL_DIR/state.db --apply

# 3. 驗證可讀 + schema 對（VPS 預設沒裝 sqlite3 CLI，用 python stdlib）
python3 -c "
import sqlite3
db = sqlite3.connect('$DRILL_DIR/state.db'.replace('\$DRILL_DIR', '$DRILL_DIR'))
print('agent_memory:', db.execute('SELECT COUNT(*) FROM agent_memory').fetchone()[0])
print('approval_queue by status:')
for s, n in db.execute('SELECT status, COUNT(*) FROM approval_queue GROUP BY status'):
    print(f'  {s}: {n}')
"

# 4. 在 vault `Incidents/YYYY/MM/drill-YYYY-MM-DD-state-restore.md` 記下：
#    - wall-clock time（每步起訖）
#    - 哪一步卡住、哪些 doc 過期
#    - frontmatter `severity: drill` + `status: closed`
#    - 用 docs/templates/incident-postmortem.md 為起點
```

**Alt 模式：Mac/桌機端**（若 VPS 不可用，e.g. drill VPS 本身的部署流程）：

Prerequisites：Mac `.env` 須有 R2 read credentials（VPS `.env` 內 `NAKAMA_R2_READ_ACCESS_KEY_ID` / `NAKAMA_R2_READ_SECRET_ACCESS_KEY` 抄過來，或 `R2_ACCOUNT_ID` + `R2_ACCESS_KEY_ID` + `R2_SECRET_ACCESS_KEY` + `R2_BUCKET_NAME` + `NAKAMA_R2_BACKUP_BUCKET` 一組完整）。Mac 預設 `.env` 這幾個 key value 是空 string（backup 只在 VPS 跑），首次需手動同步。

```bash
cd /Users/shosho/Documents/nakama
DRILL_DIR=/tmp/dr-drill-$(date +%Y%m%d) && mkdir -p $DRILL_DIR
.venv/bin/python scripts/restore_from_r2.py list --db state --limit 5
.venv/bin/python scripts/restore_from_r2.py restore --db state --target $DRILL_DIR/state.db --apply
# 後續 verify 步驟同 VPS 模式
```
