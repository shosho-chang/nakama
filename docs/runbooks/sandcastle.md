...
> - **目標 clone** `E:
akama-sandcastle` 設 `core.autocrlf false`，開跑前切到最新 `origin/main`（本機分支 `sandcastle/base`）；跑完 commit 在 `sandcastle/base`，host 端推成 `sandcastle/<issue>-<slug>` 分支開 PR
...
# 2026-09-25 起走訂閱（ADR-070）：先在本機跑 `claude setup-token` 拿一年期 token，手動寫進 .sandcastle/.env
#   CLAUDE_CODE_OAUTH_TOKEN=<token>        ← 不要貼進 chat
# 不要再放 ANTHROPIC_API_KEY（它會壓過 OAuth token，且該帳號沒有額度）
