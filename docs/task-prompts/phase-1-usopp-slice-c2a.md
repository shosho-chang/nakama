...
3. **`tests/e2e/test_phase1_publish_flow.py`** — 最少一個 test（可多個）：
   ...
   - 不做 WP-side teardown — unique `draft_id` + slug 避免跨跑互撞，整個容器 `docker compose down -v` 時一起清（`WordPressClient` 目前無 `delete_post` 方法，加進 API 表面是 Phase 2 scope）
...
- ❌ **LiteSpeed Day 1 實測** — 在 Docker WP 跑不出真實 LSCache 行為（沒有 LSWS），等 Slice C2b 在 VPS 做
