...
├── Journals/             🔒 Human only (Weekly/ → 🟡 per ADR-039)
│   ├── Daily/              Obsidian Daily Notes plugin + tpl-daily-journal
│   ├── Weekly/             🟡 Collab (ADR-039) — Bridge `/bridge/weekly`; {start-Sunday}.md; tpl-weekly-journal
│   ├── Quarterly/          人寫
│   └── Yearly/             人寫
│
...
| `Journals/Weekly/{YYYY-MM-DD}.md` | 🟡 | 修修 via Bridge Weekly Dashboard `thousand_sunny/routers/bridge_weekly.py` (body prose) + Bridge machine (frontmatter cache `start_date`/`end_date`/`status`/`top3`/`next3`/planned-actual); or Obsidian `tpl-weekly-journal` | Bridge `/bridge/weekly`, Obsidian render | ADR-039 (field-level contract; Sunday-keyed; **first 🔒→🟡 carve-out**) |
...
| `TaskNotes/Tasks/` | 🟡 | Bootstrap + `gateway/handlers/nami.py:1002` + TaskNotes plugin + **Bridge** `shared/project_writer.py` (`timeEntries[]` ADR-031 D6; `plan[]` + `scheduled` auto-sync ADR-039 D4) | TaskNotes plugin queries; Bridge weekly aggregation (`plan[]` planned + `timeEntries[]` actual) | `shared/lifeos_writer.py:render_task` + ADR-039 D4 (`plan: [{date,pomodoros,reason?}]`) |
...
- [ADR-039](decisions/ADR-039-lifeos-weekly-dashboard.md) — **Tier B Weekly Dashboard**; first 🔒→🟡 carve-out (`Journals/Weekly/`); task `plan[]` multi-date schema
