...
| `Attachments/packaging/{episode}/approval.json` | 🤖 Bridge | Bridge approve/variant/compose endpoints（ADR-054 D11；與 packages.json 分檔避免讀寫互踩） | 發布層（讀 primary_package） | rotation：approve 後重生 packages 須先 `_archive/{ts}/` 舊組（照 projects thumbnail 慣例），不同名覆寫。`reject_note` / `revision_job` 自 2026-09-15 起只讀不寫（Reject 退場，欄位留著讓舊檔仍通過 `extra="forbid"` 驗證） |
...
| `Attachments/packaging/{episode}/revisions/{request_id}/` | — | 無（2026-09-15 起沒有任何 writer：Reject 與 `revision_job` 一併退場） | — | 歷史目錄，既有的留著可回復，不再新增 |
