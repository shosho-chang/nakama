...
_PHASES = ("director", "dp", "semantic_audit")
...
def _load_execution_receipt(
    ...
    *,
    ...
    for stream_name in ("stdout", "stderr", "phase_input"):
...
def _phase_contract(phase: str) -> str:
    names = {
        "director": visual_pipeline.DIRECTOR_PLAN_CONTRACT,
        "dp": visual_pipeline.DP_FULFILLMENT_CONTRACT,
        "semantic_audit": visual_pipeline.SEMANTIC_AUDIT_CONTRACT,
    }
    return names[phase]
...
- Keep every generated/downloaded candidate inside this working directory and reference it with
  an episode-relative path. Do not request --add-dir or broader sandbox access.
...
def _execute_phase(
    ...
    *,
    ...
    if proposal_path.exists():
        raise VisualPipelineOrchestrationError(
            f"orphan {phase} proposal has no trusted execution receipt"
        )
    ...
    if forbidden_session_id is not None and result.session_id == forbidden_session_id:
        raise VisualPipelineOrchestrationError("DP session must be distinct from Director session")
    if _identity(episode_root, phase_input_path) != phase_input_before:
        raise VisualPipelineOrchestrationError(f"{phase} modified its deterministic phase input")
    proposal_identity = _proposal_identity(episode_root, proposal_path)

    receipt_dir = job_root / "receipts"
    stdout_path = receipt_dir / f"{phase}.stdout.jsonl"
    stderr_path = receipt_dir / f"{phase}.stderr.txt"
...
def run_visual_pipeline(
    ...
    *,
    ...
    for _step in range(4):
        ...
        director = visual_pipeline.load_director_plan(
            root,
            cut_id=cut_id,
            revision_id=revision_id,
            editorial_master=editorial_master,
        ...
        dp_receipt = _load_execution_receipt(
            ...
            phase="dp",
            role="dp",
            proposal_path=job_root / "workers" / "dp-session" / "dp-proposal.json",
        )
