"""Promote MEP textbook ingest from KB/Wiki.staging/ to KB/Wiki/.

Mechanical (no LLM) promotion script — same shape as promote_acsm_to_live.py:

1. 25 chapter source pages — copy KB/Wiki.staging/Sources/Books/muscle-and-
   exercise-physiology-zoladz/ → KB/Wiki/Sources/Books/muscle-and-exercise-
   physiology-zoladz/
2. Concept pages — for each staging concept whose mentioned_in includes any
   MEP ch entry:
     - If NOT in live KB/Wiki/Concepts/ → copy staging → live (new MEP concept)
     - If IN live → mechanical YAML merge: union mentioned_in (preserve live's
       body + other FM fields, only add MEP entries that aren't already there)
3. Append entry to KB/log.md

Run with --dry-run first to preview, then without to apply. Idempotent.

Usage:
    python -m scripts.promote_mep_to_live --dry-run
    python -m scripts.promote_mep_to_live
"""
...
MEP_BOOK_ID = "muscle-and-exercise-physiology-zoladz"
STAGING_BOOK_DIR = VAULT_ROOT / "KB" / "Wiki.staging" / "Sources" / "Books" / MEP_BOOK_ID
LIVE_BOOK_DIR = VAULT_ROOT / "KB" / "Wiki" / "Sources" / "Books" / MEP_BOOK_ID
...
MEP_MENTIONED_PATTERN = re.compile(rf"Sources/Books/{re.escape(MEP_BOOK_ID)}/")
...
def _mep_concept_slugs(verbose: bool = False) -> list[Path]:
    """Return staging concept .md paths whose mentioned_in includes any MEP entry."""
    out: list[Path] = []
    for p in STAGING_CONCEPTS_DIR.glob("*.md"):
        try:
            ...
            if any(MEP_MENTIONED_PATTERN.search(str(m)) for m in mentioned):
                out.append(p)
...
def promote_concepts(*, dry_run: bool) -> tuple[int, int, int]:
    """For each MEP-mentioned staging concept: copy if new, merge mentioned_in if existing.
    ...
    print(f"  ({len(staging_files)} staging concepts mention MEP — processing)")
    for src in staging_files:
        ...
        merged_count += 1
        added_mep_only = [m for m in added if MEP_MENTIONED_PATTERN.search(str(m))]
        print(f"  [concept    MERGED] {src.name}  (+{len(added_mep_only)} MEP entries)")
        if not dry_run:
...
def append_log(*, ch_new: int, ch_overwrite: int, c_new: int, c_merged: int, dry_run: bool) -> None:
    ...
    pipeline_note = (
        "ch1-25 via L3 CLI Max Plan with NAKAMA_CLAUDE_CLI_TIMEOUT=1800 "
        "(13 chapters >100K chars; biggest ch10/ch22 ~200K)"
    )
    entry = f"""
## {today} — MEP textbook v3 ingest promoted to live

- 25 chapter source pages → KB/Wiki/Sources/Books/{MEP_BOOK_ID}/ ({ch_summary})
- {c_new} new concept pages → KB/Wiki/Concepts/
- {c_merged} existing concept pages merged (BSE/SN/ACSM+MEP mentioned_in union)
- All 25 chapters pass 7-condition acceptance gate after C5 cleanup
...
def main() -> int:
    ...
    print(f"  Concepts: {c_new} new + {c_merged} merged + {c_unchanged} unchanged (no MEP delta)")
    append_log(
        ch_new=ch_new,
