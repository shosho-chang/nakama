...
from shared.thumbnail_person_preview import render_person_placement_preview  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument(
        "--preview-archetype",
        default="",
        help="Optional overlay archetype, e.g. tv3-split, for a top-candidate person preview.",
    )
    ...
    if args.preview_archetype:
        preview = render_person_placement_preview(
            cutout_path=candidates[0].path,
            out_png=out_dir / "person_placement_preview.png",
            archetype=args.preview_archetype,
            cutout_id=candidates[0].cutout_id,
            pose_tags=candidates[0].tags,
        )
