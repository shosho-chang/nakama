"""Promote Benardot ACSM Nutrition for Exercise Science from KB/Wiki.staging/ to KB/Wiki/.

Mechanical (no LLM) promotion script — same shape as promote_mep_to_live.py:

1. 15 chapter source pages — copy KB/Wiki.staging/Sources/Books/acsm-nutrition-
   for-exercise-science-benardot/ → KB/Wiki/Sources/Books/acsm-nutrition-for-
   exercise-science-benardot/
2. Concept pages — for each staging concept whose mentioned_in includes any
   Benardot ch entry:
     - If NOT in live KB/Wiki/Concepts/ → copy staging → live (new BENARDOT concept)
     - If IN live → mechanical YAML merge: union mentioned_in (preserve live's
       body + other FM fields, only add BENARDOT entries that aren't already there)
3. Append entry to KB/log.md

Run with --dry-run first to preview, then without to apply. Idempotent.

Usage:
    python -m scripts.promote_benardot_to_live --dry-run
    python -m scripts.promote_benardot_to_live
"""
...
BENARDOT_BOOK_ID = "acsm-nutrition-for-exercise-science-benardot"
STAGING_BOOK_DIR = VAULT_ROOT / "KB" / "Wiki.staging" / "Sources" / "Books" / BENARDOT_BOOK_ID
LIVE_BOOK_DIR = VAULT_ROOT / "KB" / "Wiki" / "Sources" / "Books" / BENARDOT_BOOK_ID
...
BENARDOT_MENTIONED_PATTERN = re.compile(rf"Sources/Books/{re.escape(BENARDOT_BOOK_ID)}/")
...
def _mep_concept_slugs(verbose: bool = False) -> list[Path]:
    """Return staging concept .md paths whose mentioned_in includes any BENARDOT entry."""
    out: list[Path] = []
    for p in STAGING_CONCEPTS_DIR.glob("*.md"):
        try:
            ...
            if any(BENARDOT_MENTIONED_PATTERN.search(str(m)) for m in mentioned):
                out.append(p)
...
def promote_concepts(*, dry_run: bool) -> tuple[int, int, int]:
    """For each BENARDOT-mentioned staging concept: copy if new, merge mentioned_in if existing.
    ...
    merged_count = 0
    unchanged_count = 0
    staging_files = _mep_concept_slugs()
    print(f"  ({len(staging_files)} staging concepts mention BENARDOT — processing)")
    for src in staging_files:
        ...
        merged_count += 1
        added_mep_only = [m for m in added if BENARDOT_MENTIONED_PATTERN.search(str(m))]
        print(f"  [concept    MERGED] {src.name}  (+{len(added_mep_only)} BENARDOT entries)")
        if not dry_run:
...
def append_log(*, ch_new: int, ch_overwrite: int, c_new: int, c_merged: int, dry_run: bool) -> None:
    ...
    pipeline_note = (
        "ch1-15 via L3 CLI Max Plan with NAKAMA_CLAUDE_CLI_TIMEOUT=1800 "
        "(all chapters 88K-190K chars)"
    )
    entry = f"""
## {today} — Benardot (ACSM Nutrition for Exercise Science) v3 ingest promoted to live

- 15 chapter source pages → KB/Wiki/Sources/Books/{BENARDOT_BOOK_ID}/ ({ch_summary})
- {c_new} new concept pages → KB/Wiki/Concepts/
- {c_merged} existing concept pages merged (BSE/SN/ACSM/MEP+Benardot mentioned_in union)
- All 15 chapters pass 7-condition acceptance gate after C5 cleanup
...
def main() -> int:
    ...
    print(f"=== BENARDOT promotion ({mode}) ===\n")
    ...
    print(
        f"  Concepts: {c_new} new + {c_merged} merged + {c_unchanged} unchanged (no Benardot delta)"
    )
    append_log(
        ch_new=ch_new,
