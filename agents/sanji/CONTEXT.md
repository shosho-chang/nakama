...
**營運分工（Q15，2026-08-23 定案）**。修修將聘**社群經理**（人類）承接實體與儀式面，
角色四分——社群經理／修修／Sanji／內容管線：

| 負擔 | 歸屬 | 說明 |
|---|---|---|
| 實體聚會 | **社群經理（將聘）** | 長期輔以 Lv.10+ 會員主辦資格（privilege 方向不變） |
| 挑戰開始／結束直播（W2、W12–13） | **社群經理** | 賽季儀式主持；功能導覽建議錄成 evergreen 課減少每季重複（其營運裁量） |
| 每月 2 場直播＝**讀書會**＋**身心健康趨勢** | **修修** | 與 YouTube 頻道主題 aligned |
| mini course 製作 | **修修錄、管線剪** | 直播中段以第二機位錄主題講解 → AI Agent 剪輯＋上字幕 → 成課。**內容飛輪：一次直播、多處輸出（直播／YouTube／課程）** |
| 每週身心健康研究貼文 | 研究管線衍生 | 研究一次、多處輸出；成文／排程走管線、修修核可（※此行為 Claude 解讀，修修可修正） |
| 所有 bookkeeping ＋程式判定觸發 | **Sanji** | 判定、帳本、榜單、對帳、開船週召集、驚喜發放、隨機題投放（Q16） |
| 挑戰主題與導論課內容 | **修修** | IP 本體，不可轉移 |

- 修修對 Sanji 的定位原話：「工具就是做其他所有 bookkeeping 的工作，以及做一些程式判定
  會觸發的事情。」
- **架構前置**：HITL 裁決佇列、退件溝通等營運介面的權限做成**可指派角色（operator role）**，
  不硬編修修——社群經理上任後可承接部分裁決，「每日介入＝設計失敗」多一道人類洩壓閥。
...
**隨機式 Engagement（Q16，2026-08-23 定案）**。「隨機」在系統中的四個可能位置與裁決：

| 型 | 裁決 |
|---|---|
| 隨機知識題（登入練習題） | ✅ 入列，Phase 2（設計見下） |
| 驚喜獎勵（S1–S5） | ✅ 已定案（驚喜引擎） |
| 隨機加成事件（「本日打卡雙倍」隨機降臨） | ⏸ PARK 到第二年——經濟先穩跑一年。屆時用隨機不用排定（排定誘發策略性等待，隨機無法等） |
| 抽獎／Mystery Box（貝里下注） | ⛔ **永久出局（憲法禁令）** |

**隨機憲法（四條）**：
1. **隨機只出現在禮物端，永不出現在付費端**——會員貝里永不進入機率事件
   （賭博機制與身心健康品牌互斥；「可能損失已付出的」是純 Black Hat CD8）。
2. **隨機的是「出現與否」，不是「獎多大」**——獎額固定小額；變動獎額＝吃角子老虎
   核心機關（variable-ratio magnitude），刻意不用。
3. **不可 re-roll、不可用重複行為增加抽取次數**——觸發只掛已封頂行為（登入一天一次）。
4. **拿掉獎勵仍值得發生**——每次隨機互動須內含真實價值；純拉霸空變異不設。

**登入練習題（Phase 2）** = 社群的**環境式間隔重複層**：課程內容因遺忘曲線衰退，
隨機題讓教學內容全年保持活性（retrieval practice／測試效應）。
- **答對** +10 XP／1 貝里（placeholder，進 `level_curve_sim.py` 校準；潛水型年增約
  +500 XP、佔比 ~12%，不擾動等級曲線）；**答錯零懲罰＋當場解說**（失敗檢索＋回饋
  仍有學習效果），同概念數週後再出現（間隔重複本來就要重考）
- 頻率：每人每日最多 1 題、平均每週隨機 2–3 次；**per-member 隨機＋題目個別抽選**
  （全站同日同題＝答案十分鐘就在聊天室外流；個別抽選讓外流無利可圖，且外流的
  「看答案」＝讀了一則健康知識，通過驚喜引擎的洩漏安全判準）
- 題庫：Sanji 從既有內容起草（mini course／研究貼文／讀書會書目）→ 修修批次審核入庫
  （零產出負荷，與訊息治理同模式）。100–200 題可跑一年
- 憲法相容：答案系統驗證（🟢）＋觸發掛登入上限（🟡），雙判準通過
- 實作註記：「登入跳出」需 portal 前端注入點（FluentCommunity Vue portal），
  可行性列入 contract probe；fallback＝portal widget＋鈴鐺通知
