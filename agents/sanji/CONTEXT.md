...
> ⛔ **貝里已完全退役（修修 2026-09-02 裁決）。** 程式、文案、REST payload 與資料庫
> 欄位全數移除（`migrations/003-drop-berry.php`）。本文件下方所有提到貝里的段落
> ——雙數字經濟、季末商店、定價量級、回覆格式、「暫不顯示」——**都是歷史裁決記錄**，
> 不是現行機制。保留它們是為了讓「為什麼曾經走雙貨幣、又為什麼收掉」可追溯；
> 日後若有人再提第二種貨幣，請先讀那幾段。**不得據以復活任何貝里行為。**
>
...
**分數表 v6**（A＋B 裁決，目標＝十年登頂）：打卡 10→20、全勤 200→500、整門課 300→500；
...
**貝里退役**（v1.2 封存 → 2026-09-02 完全移除）：先前是「帳本照記、介面不出現」，
修修 2026-09-02 裁決改為從實作徹底拿掉——`berry_of()`、grant payload 欄位、回覆文案
slot、REST 欄位、`grants.berry` 與 `balances.berry_balance` 兩個資料庫欄位全刪。
無損性已證：貝里恆等於 XP÷10，上線 2,294 筆授予零例外。福利階梯取代商店；
已公告的福利只加不減。

**位階線 v4**（2026-09-02）：霸王色與傳說船長退場，剩超新星／最惡世代／王下七武海／
四皇／海賊王五階；level 對映待修修重新確認。**航線**：佐烏退場，香波地群島接 Lv.11、
魚人島順延 Lv.12；仍是 16 站、門檻未動。
