...
**Promotion package** = `agents/robin/promotion/`（ADR-050）— Source Promotion domain logic 的 bounded package。原散在 `shared/` 平面命名空間的 19 個 promotion 模組（preflight / source map builder / concept + entity engines / commit / acceptance gate / renderer / targets / dry-run fixtures / KB indexes / resolver / video 兩支）收攏於此。`shared/` 只留 2+ agent 共用的基礎設施；promotion 的 **contract types** 仍在 `shared/schemas/`（presentation 與 domain 共用的 interface 層，不搬）。
_Avoid_: 把新的 promotion 模組放回 `shared/` 頂層。

**Promotion composition root**（組裝工廠）= `agents.robin.promotion.factory` — Robin 擁有 domain logic **也擁有它的組裝成本**。`load_promotion_config()`（唯一 env 讀取點，W6 boundary）＋ `build_promotion_review_service(config)`（mode 分支 + 9-service collaborator graph → 組裝好的 `PromotionReviewService`）。Thousand Sunny 的 `promotion_wiring.py` 是 thin shim：載入 config → 呼叫 factory → `set_service()` 注入 router。CLI / 未來 agent 走同一個工廠，不得複製組裝邏輯。
_Avoid_: 在 presentation layer 手工組裝 promotion service；在 factory 之外散落 `os.getenv` 讀取。
