...
from shared.epub_metadata import (
    MalformedEPUBError,
    build_toc_title_map,
    extract_metadata,
    extract_spine_items,
...
_logger = get_logger("nakama.agents.robin.promotion.source_map_builder")
...
class SourceMapBuilder:
    ...
    def _inspect(
        ...
        if reading_source.kind == "youtube_video":
            ...
            # dedicated bypass-engine path (``agents.robin.promotion.video_source_map_builder
            ...
            raise _BuildError(
                ...
                "agents.robin.promotion.video_source_map_builder.build_video_source_map; "
    ...
    def _inspect_ebook(
        ...
        try:
            spine_items = extract_spine_items(blob)
        ...
        toc_titles = build_toc_title_map(metadata.toc)
...
def _strip_xml(xhtml: str) -> str:
    ...
    Mirrors ``agents.robin.promotion.promotion_preflight._strip_xml``."""
