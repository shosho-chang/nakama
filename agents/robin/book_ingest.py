...
Why the EN original, not the bilingual EPUB: bilingual EPUBs interleave EN+ZH
paragraphs and summarize poorly; the original is clean single-language. The
enqueue endpoint already requires ``has_original``, so it exists. The Chinese-KB
concept extraction reads it the same way it would an English article.
...
def ingest_one(book_id: str, *, vault: Path | None = None) -> None:
    """Read book ``book_id``'s EN EPUB → flatten → run ``IngestPipeline`` (source_type=book).

    Raises on failure so the caller can mark the queue row ``failed``.
    """
    ...
    if not book.has_original:
        raise EPUBTextError(f"book {book_id!r} has no original.epub to ingest")

    blob = read_book_blob(book_id, lang="en")
