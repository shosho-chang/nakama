...
def _yaml_safe(value: str) -> str:
    """Strip CR/LF to prevent YAML injection across multi-line frontmatter values.

    Tighter than json.dumps which would also escape unicode — we want raw UTF-8
    in the file (中文 titles must stay readable).
    """
    ...
    def _serialise_frontmatter(
        *,
        ...
        lines = [
            ...
            f'source: "{_yaml_safe(original_url)}"',
            f'original_url: "{_yaml_safe(original_url)}"',
            ...
            f'fulltext_source: "{_yaml_safe(fulltext_source)}"',
        ]
        if note:
            lines.append(f'note: "{_yaml_safe(note)}"')
        lines.extend(["---", ""])
        return "\n".join(lines) + "\n"
