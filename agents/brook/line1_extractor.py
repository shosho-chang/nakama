...
- episode_type 必須是：narrative_journey / myth_busting / framework / listicle
...
class Line1Extractor:
    ...
    def extract(self, source_input: str, metadata: EpisodeMetadata) -> Stage1Result:
        ...
        for attempt in range(2):
            if attempt > 0:
                logger.warning(
                    "Stage 1 schema validation failed (attempt %d/2); retrying", attempt + 1
                )
            raw = ask_multi(messages, system=system, model=_MODEL, max_tokens=4096)
            try:
