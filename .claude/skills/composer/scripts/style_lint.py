...
_PLACEHOLDER = re.compile(
    r"【(修修素材|研究來源待補|來賓語錄待補|時效事實待確認)[：:]([^】]{0,160})】"
)

# 文類門檻：(zh 字數 WARN 下限, 助詞 FAIL 上限, 助詞 WARN 帶, ！FAIL 上限, ！WARN 帶, 導言必須, CTA 預期)
_GENRE = {
    "book-review": dict(
        zh_floor=4500, p_hi=45, p_band=(10, 30), b_hi=30, b_band=(5, 20), lede=True, cta=True
    ),
    "science": dict(
        zh_floor=4500, p_hi=45, p_band=(10, 30), b_hi=30, b_band=(5, 20), lede=True, cta=True
    ),
    "people": dict(
        zh_floor=1000, p_hi=25, p_band=(2, 15), b_hi=15, b_band=(1, 8), lede=False, cta=False
    ),
}
...
def lint(path: Path, genre: str) -> int:
    ...
    if g["cta"]:
        ...
        add(
            ...
            "有" if has_cta else "結尾 1500 字內沒看到電子報/分享 CTA",
        )

    add(
        "篇幅",
        "WARN" if zh < g["zh_floor"] else "PASS",
        f"{zh} 中文字（{genre} 下限參考 {g['zh_floor']}）",
    )
    add(
        "粗體重音",
        "WARN" if bold_pairs == 0 else "PASS",
        f"{bold_pairs} 處（金句/關鍵數字該有 bold）",
    )
    ...
    add(
        "佔位符",
        "WARN" if stray > 0 else "INFO",
        f"合法 {len(phs)} 個"
        + (f"；疑似格式不符的【 {stray} 個" if stray > 0 else "")
