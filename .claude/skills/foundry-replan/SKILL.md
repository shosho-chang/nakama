...
description: >
  ...
  review. Triggers on /foundry-replan or "重 plan beat <N>" style requests.
  Loads agents/foundry/prompts/broll_planner.md as canonical prompt, runs LLM
  on a single beat with user-supplied note, returns new proposal for inline
  approval. Canonical pipeline logic is in agents/foundry/ (Python); this skill
  is escape hatch only.
---

# foundry-replan — single-beat re-plan helper

You are an interactive surface for re-planning a single storyboard beat during conversation review. The canonical broll pipeline lives in `agents/foundry/` (Python). This skill is **escape hatch only** — Brook's UI normally handles re-plans via `/foundry/<episode>` Bridge route + endpoint. Use this skill when the user wants to iterate on a beat in conversation rather than via UI.

## When to use

Trigger on:
- `/foundry-replan` slash invocation
- 「重 plan beat 12」「這個 beat 換個 layout 試試」
- 「foundry 的 beat 7 我想 calmer」

Do NOT trigger for:
- Bulk replanning (use UI batch actions)
- Initial storyboard generation (use `python -m agents.foundry plan --episode <id>`)
...
2. **Load canonical prompt** — `agents/foundry/prompts/broll_planner.md` (the planner's normal prompt template, PR-3 implements). Load `agents/foundry/STYLE.md` editorial rubric. Load `agents/foundry/guardrails.yaml`.
...
7. **Write back** — if confirmed:
   ...
   - Append entry to `agents/foundry/edit_log/<episode>.jsonl` with `{beat_id, before, after, user_note}`
