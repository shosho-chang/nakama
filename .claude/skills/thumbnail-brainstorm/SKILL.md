...
| **頂緣錨定** | 每張照片**自己的頭頂**釘畫布頂緣（crown_screen=0）→ headroom 結構性歸零；下緣搆不到畫布底＝裁切框太短，fail loud 回去改 crop | 眼線硬鎖正是 headroom 的根源（教訓 25）；TF 規格＝頂天蓋地 |
| 眼線 | **軟目標**：solver 輸出預測眼位，render QA 驗「實測 vs 預測 ≤10px」；跨張隨姿勢漂＝合法 | TF 原版自己容忍眼線漂 ~10% canvas；把它當硬約束就會擠出 headroom |
