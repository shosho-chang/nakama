...
### `split_opener_sec = 0` 的短片（2026-09-12，20260721 呂冠緯）

**上面兩個 anchor 都是以接縫 y=960 為基準算的。沒有分割開場時它們一律是錯的**
——(320,592)–(760,952) 在滿版談話鏡頭裡正好落在臉的正中央，實測蓋住主持人的
嘴與下巴，踩的就是他 2026-08-30 講的同一件事。不要因為「前一集就是這個數字」
照抄。

這種短片改用 `--anchor free --card-x <x> --card-y <y>`（卡片左上角，畫布座標），
落點**每集自己量**——乾淨的位置取決於主持人坐在畫面的哪一側，沒有跨集通用常數。
量法：拿 `--stills` 出來的那一格全解析度畫面，用 `drawbox` 把候選矩形疊上去看，
確認同時避開三條帶：

| 帶 | 範圍 | 來源 |
|---|---|---|
| 人臉／頭髮 | 逐集量 | 談話鏡頭本身 |
| 字卡帶 | `pos_y` 0.58–0.66（y≈1114–1267） | `run_short_titles` 的 `FORMAT_TITLES` |
| 字幕帶 | 0.88 起 | 同上 |

20260721 呂冠緯 三支的解：主持人坐畫面左側，右側整條從頂到桌面都是植栽背景，
落點取 `--card-x 580 --card-y 200`（卡片 (580,200)–(1020,560)）。白卡黑線稿壓在
深色植栽上對比很夠，且避開手機平台頂部與右側的 UI 疊層。
