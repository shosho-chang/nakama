...
**第 3 測「不是複述」2026-09-14 起也由程式擋**（`_hero_text.is_verbatim_quote`，
在 `_engine._events_for_acceptance` 的 Director 驗收鏈上）：卡片文字與它自己宣告的
master cue 逐字稿，各自抽掉標點、空白、語助詞與第三人稱之後，若卡片是逐字稿的**連續
子字串**就退件。

為什麼需要它——這條規則本來就在 Director 的 prompt 裡（`_STAGE_SKILL_MANUALS` 會把本
手冊全文接進去，9/14 實際送出 57,277 字，四句判準全在），Director 讀了，然後同一集
punch-L02／L03 的四張 hero 仍然全是逐字複述。**同一個症狀 9/08 就記過一次**
（`_codex_semantic.py` 註解：「Hero 只是把字幕放大」），當時的處方就是把手冊接進
prompt——接上去了，六天後原封不動再來一次。prompt 層的指示守不住這條。

判準刻意用子字串而不是模糊相似度：上面判「留」的「一天六七千個念頭都在罵自己」跟原文
共用大量字元，相似度會誤擋它；而它做的是跨段濃縮＋換詞（罵 vs 批判），子字串測試放行。
**誤擋好卡片比放過壞卡片更糟**——前者會逼人繞過這道閘，後者至少還有人眼在 review。
