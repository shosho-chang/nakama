...
6. **報告落點**：預設受審文件同層，一輪 `TA審稿回饋-{對象}-{日期}.md`、二輪 `TA審稿-二輪評分-{對象}-{日期}.md`（與黃金範例同慣例）。**例外**：受審文件位於 Obsidian vault 內時，報告不寫入 vault（`Journals/` 禁寫、`KB/Raw/` 不可污染）——改存專案工作資料夾或問使用者。
...
Prompt 用 [references/prompt-templates.md](references/prompt-templates.md)（一輪模板 A、二輪模板 B、研究員模板 C、多版本盲選模板 D），代入 persona 全文、閱讀指示、rubric 的「輸出要求」。組 prompt 時逐項檢查：
...
- **卻步點**：rubric 的輸出要求都含「即使高分仍列 1–3 個卻步點」——確認組 prompt 時沒被裁掉。防好好先生；9 分的卻步點往往是最後一哩的精確地圖。
...
**圖像 artifact**（PNG 圖卡）沒有可比對的原文，script 不適用——查證改為人工抽核：persona 描述的每個畫面元素（文案字句、卡號、視覺細節）逐條對照圖檔確實存在，虛構同樣整份作廢。
發現虛構 → 該 persona **整份作廢**，換個說法重下 prompt 重跑，並記入報告「驗證備註」。
**完成判準**：每條引句（或每條畫面描述）有 pass／近似／虛構 判定，每個 persona 有查證結論。沒跑完這步，報告不可交付。
...
查證步驟（Step 4）照跑。報告用二輪模板（見 report-templates），其中**最後一哩清單**把剩餘扣分分成：可再修的／只有委託人能拍板的／行銷層的——讓委託人一眼知道下一步在誰手上。
**完成判準**：每條前次疑慮都有三態標記＋新版小節引證。

### 7. 多版本盲選（輪次＝盲選時）

每 persona 對每版獨立盲評（用模板 D——prompt 絕不含版本序）後，把分數整理成 JSON 餵 `scripts/score_ledger.py`：分數軌跡表、per-unit 表、drop-off 彙整、argmax 與 blind pairwise 交叉檢查。`--plateau-delta` 依 rubric 量表挑（100 分制約 2、0–10 制約 0.5）。兩版無明顯差異或 pairwise 平手時**如實回報 plateau／平手，不硬選**。
