"""Foundry storyboard Bridge UI Tier 2 (ADR-032 §7, PR-5).

GET  /foundry/{episode_id}                    storyboard table
GET  /foundry/{episode_id}/status              JSON poll (chips, every 2.5s)

POST /foundry/{episode_id}/beat/{bid}/approve  text_approved=True + enqueue
POST /foundry/{episode_id}/beat/{bid}/edit     layout/component/params edit
POST /foundry/{episode_id}/beat/{bid}/replan   re-plan via LLM + edit_log
...
logger = get_logger("nakama.web.foundry")

page_router = APIRouter(prefix="/foundry", tags=["foundry"])

_TEMPLATE_DIR = Path(__file__).resolve().parent.parent / "templates" / "foundry"
...
def _shosho_asset_version() -> str:
    ...
    foundry_js = Path(__file__).resolve().parent.parent / "static" / "foundry" / "foundry.js"
    if foundry_js.exists():
        h.update(foundry_js.read_bytes())
...
async def _default_render_beat(episode_id: str, beat_id: int) -> None:
    ...
    """
    from agents.foundry.render_dispatcher import dispatch_beat
...
def _auth_or_redirect(nakama_auth: str | None, episode_id: str) -> RedirectResponse | None:
    if check_auth(nakama_auth):
        return None
    return RedirectResponse(f"/login?next=/foundry/{episode_id}", status_code=302)
...
def _redirect_back(episode_id: str) -> RedirectResponse:
    return RedirectResponse(f"/foundry/{episode_id}", status_code=303)
